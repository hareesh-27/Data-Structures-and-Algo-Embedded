## We will see key point from both include (.h) and source (.c) files

**First is #ifndef and #endif**
  If multiple files include the same header (directly or indirectly), the header may get included multiple times during preprocessing, 
...
**First :**
We use typedef with struct to create a new type name so that we can use the structure without writing the struct keyword every time,
